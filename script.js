...
const DB_NAME = "scada_supervisorio_db_v22";
const DB_VERSION = 1;
...
function saveRecord(db, key, value) {
  return new Promise((resolve, reject) => {
...
async function saveAllData() {
  try {
    ...
    await saveRecord(db, "equipment", equipmentState);
    await saveRecord(db, "ongoing", ongoingActions || []);
    await saveRecord(db, "history", actionHistory || []);
    db.close();
  } catch (error) {
    console.error("Erro ao salvar IndexedDB:", error);
  }
}
...
async function loadAllData() {
  try {
    const db = await openDatabase();
    const equipmentState = await loadRecord(db, "equipment");
    const ongoingState = await loadRecord(db, "ongoing");
    const historyState = await loadRecord(db, "history");
...
async function finalizeActionById(actionId) {
